const data = {
    users: [
        {
            id: 1,
            age: 29,
            name: "Arek",
            sex: "male"
        },
        {
            id: 2,
            age: 49,
            name: "Marta",
            sex: "female"
        },
        {
            id: 3,
            age: 19,
            name: "Stasia",
            sex: "female"
        },
        {
            id: 4,
            age: 24,
            name: "Karol",
            sex: "male"
        }
    ]
}

const Item = ({el}) => (
    <div>
        <h1>{el.name}</h1>
        <p>Informacje o użytkowniku</p>
        <p>Wiek użytkownika: <strong>{el.age}</strong></p>
        <p>Płeć użytkownika: <strong>{el.sex}</strong></p>
    </div>
)







class ListItems extends React.Component {

    state = {
        select: "all",
    }


    usersList = () => {
        let users = this.props.data.users
        switch (this.state.select) {
            case "all" :
                return users.map(el => <Item el={el} key={el.id}/>)

            case "female":
                users = users.filter(el => el.sex === 'female')
                return users.map(el => <Item el={el} key={el.id}/>)

            case "male":
                users = users.filter(el => el.sex === 'male')
                return users.map(el => <Item el={el} key={el.id}/>)

            default :
                return "Brak danych"
        }
    }


    handleUsersFilter(option) {
        this.setState({select: option})
    }


    render() {


        return (
            <div>
                <button onClick={this.handleUsersFilter.bind(this, "all")}>Wszyscy</button>
                <button onClick={this.handleUsersFilter.bind(this, "female")}>Kobiety</button>
                <button onClick={this.handleUsersFilter.bind(this, "male")}>Mężczyźni</button>
                {this.usersList()}
            </div>
        )
    }
}

ReactDOM.render(<ListItems data={data}/>, document.getElementById('root'))
